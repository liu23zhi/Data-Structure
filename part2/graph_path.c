...
/* ─────────────────────────────────────────────
   修正：创建邻接表函数（尾插法，保持边的输入顺序）
   原错误：使用头插法导致边的顺序与输入相反
   ───────────────────────────────────────────── */
static void add_edge_list(int u, int v)
{
    ArcNode *p = (ArcNode *)malloc(sizeof(ArcNode));
    ...
    // 尾插法，保持边的输入顺序
    if (adj_list[u].first == NULL) {
        adj_list[u].first = p;
    } else {
        ArcNode *q = adj_list[u].first;
        while (q->next != NULL) {
            q = q->next;
        }
        q->next = p;
...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
// 邻接表转邻接矩阵
static void list_to_matrix(void)
{
    // 先清空现有邻接矩阵
...
// 邻接矩阵转邻接表
static void matrix_to_list(void)
{
    // 先清空现有邻接表
    for (int i = 1; i <= num_vertices; i++) {
        ArcNode *current = adj_list[i].first;
        while (current != NULL) {
            ArcNode *next = current->next;
            free(current);
            current = next;
        }
...
static int  queue_full(Queue *q)      { return (q->rear - q->front) >= QUEUE_SIZE; }
static void enqueue(Queue *q, int x)  
{ 
    if (queue_full(q)) {
        fprintf(stderr, "队列溢出错误：队列已满，无法入队元素 %d\n", x);
        exit(EXIT_FAILURE);
    }
    q->data[q->rear++ % QUEUE_SIZE] = x; 
}
static int  dequeue(Queue *q)         
{ 
    if (queue_empty(q)) {
        fprintf(stderr, "队列下溢错误：队列为空，无法出队\n");
        exit(EXIT_FAILURE);
    }
    return q->data[q->front++ % QUEUE_SIZE]; 
}
...
/* ─────────────────────────────────────────────
   新增：递归递推方式打印最短路径
   ───────────────────────────────────────────── */
static void print_path_recursive(int parent[], int dst)
{
    if (parent[dst] == -1) {
        fprintf(POUT, "%d", dst);
        return;
    }
    print_path_recursive(parent, parent[dst]);
    fprintf(POUT, " -> %d", dst);
}

/* ─────────────────────────────────────────────
   完善：迭代+递归两种方式打印path数组
   ───────────────────────────────────────────── */
static void print_path_iterative(int parent[], int dst)
{
    ...
    fprintf(POUT, "\n    最短路径（递归递推）: ");
    print_path_recursive(parent, dst);
    fprintf(POUT, "\n");
}
...
/* ─────────────────────────────────────────────
   优化：邻接表摧毁函数
   ───────────────────────────────────────────── */
static void destroy_adj_list(void)
{
    for (int i = 1; i <= num_vertices; i++) {
        ArcNode *current = adj_list[i].first;
        while (current != NULL) {
...
static void run_graph_search(int src, int dst)
{
    ...
    /* 任务3-A：递推（迭代+递归）方式打印最短路径的 path 数组 */
    fprintf(POUT, "=================================================================\n");
    fprintf(POUT, "【任务3-A】递推（迭代+递归）方式打印最短路径的 path 数组\n");
    fprintf(POUT, "=================================================================\n");
    if (dist_list >= 0) {
    ...
    /* 任务4：邻接矩阵与邻接表相互转换演示 */
    fprintf(POUT, "=================================================================\n");
    fprintf(POUT, "【任务4】邻接矩阵与邻接表相互转换演示\n");
    fprintf(POUT, "=================================================================\n");
